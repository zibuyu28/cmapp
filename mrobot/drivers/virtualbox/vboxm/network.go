...
func (n *hostOnlyNetwork) SaveIPv4(vbox VBoxManager) error {
	if n.IPv4.IP != nil && n.IPv4.Mask != nil {
		if runtime.GOOS == "windows" {
			log.Warn(n.ctx, "Windows might ask for the permission to configure a network adapter. Sometimes, such confirmation window is minimized in the taskbar.")
		}
...
func createHostonlyAdapter(ctx context.Context, vbox VBoxManager) (*hostOnlyNetwork, error) {
	if runtime.GOOS == "windows" {
		log.Warn(ctx, "Windows might ask for the permission to create a network adapter. Sometimes, such confirmation window is minimized in the taskbar.")
	}
	...
	return &hostOnlyNetwork{Name: res[1], ctx: ctx}, nil
}
...
func getHostOnlyAdapter(ctx context.Context, nets map[string]*hostOnlyNetwork, hostIP net.IP, netmask net.IPMask) *hostOnlyNetwork {
	for _, n := range nets {
		...
		if hostIP.Equal(n.IPv4.IP) &&
			...
			log.Debugf(ctx, "Found: %s", n.Name)
			return n
		}
	}

	log.Debug(ctx, "Not found")
	return nil
}

func getOrCreateHostOnlyNetwork(ctx context.Context, hostIP net.IP, netmask net.IPMask, nets map[string]*hostOnlyNetwork, vbox VBoxManager) (*hostOnlyNetwork, error) {
	// Search for an existing host-only adapter.
	hostOnlyAdapter := getHostOnlyAdapter(ctx, nets, hostIP, netmask)
	if hostOnlyAdapter != nil {
		return hostOnlyAdapter, nil
	}

	// No existing host-only adapter found. Create a new one.
	_, err := createHostonlyAdapter(ctx, vbox)
	if err != nil {
		...
		log.Warnf(ctx, "Creating a new host-only adapter produced an error: %s", err)
		log.Warn(ctx, "This is a known VirtualBox bug. Let's try to recover anyway...")
	}
	...
	log.Warnf(ctx, "Found a new host-only adapter: %q", hostOnlyAdapter.Name)
...
func removeOrphanDHCPServers(ctx context.Context, vbox VBoxManager) error {
	...
	log.Debug(ctx, "Removing orphan DHCP servers...")
		...
		if strings.HasPrefix(name, dhcpPrefix) {
			if _, present := nets[name]; !present {
				if err := vbox.vbm("dhcpserver", "remove", "--netname", name); err != nil {
					log.Warnf(ctx, "Unable to remove orphan dhcp server %q: %s", name, err)
				}
			}
		}
	}
...
func addHostOnlyDHCPServer(ctx context.Context, ifname string, d dhcpServer, vbox VBoxManager) error {
	...
	if runtime.GOOS == "windows" {
		log.Warn(ctx, "Windows might ask for the permission to configure a dhcp server. Sometimes, such confirmation window is minimized in the taskbar.")
	}
