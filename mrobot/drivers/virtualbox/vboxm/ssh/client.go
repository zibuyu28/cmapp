...
func NewClient(ctx context.Context, user string, host string, port int, auth *Auth) (Client, error) {
	sshBinaryPath, err := exec.LookPath("ssh")
	if err != nil {
		log.Debug(ctx, "SSH binary not found, using native Go implementation")
		client, err := NewNativeClient(ctx, user, host, port, auth)
		log.Debugf(ctx, "%v", client)
		return client, err
	}

	if defaultClientType == Native {
		log.Debug(ctx, "Using SSH client type: native")
		client, err := NewNativeClient(ctx, user, host, port, auth)
		log.Debugf(ctx, "%v", client)
		return client, err
	}

	log.Debug(ctx, "Using SSH client type: external")
	client, err := NewExternalClient(ctx, sshBinaryPath, user, host, port, auth)
	log.Debugf(ctx, "%v", client)
	return client, err
}

func NewNativeClient(ctx context.Context, user, host string, port int, auth *Auth) (Client, error) {
...
func (client *NativeClient) dialSuccess() bool {
	...
	if err != nil {
		log.Debugf(client.ctx, "Error dialing TCP: %s", err)
		return false
	}
	closeConn(conn)
	return true
...
func NewExternalClient(ctx context.Context, sshBinaryPath, user, host string, port int, auth *Auth) (*ExternalClient, error) {
	...
	for _, privateKeyPath := range auth.Keys {
		if privateKeyPath != "" {
			...
			if runtime.GOOS != "windows" {
				mode := fi.Mode()
				log.Debugf(ctx, "Using SSH private key: %s (%s)", privateKeyPath, mode)
...
func closeConn(c io.Closer) {
	err := c.Close()
	if err != nil {
		log.Debugf(context.Background(), "Error closing SSH Client: %s", err)
	}
}
