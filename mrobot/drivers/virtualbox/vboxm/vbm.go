...
func NewVBoxManager(ctx context.Context) *VBoxCmdManager {
...
func (v *VBoxCmdManager) vbmOutErrRetry(retry int, args ...string) (string, string, error) {
	...
	log.Debugf(v.ctx, "COMMAND: %v %v", vboxManageCmd, strings.Join(args, " "))
	...
	if len(args) > 0 {
		log.Debugf(v.ctx, "STDOUT:\n{\n%v}", stdout.String())
		log.Debugf(v.ctx, "STDERR:\n{\n%v}", stderrStr)
	}
...
func checkVBoxManageVersion(version string) error {
	...
	if major < 5 {
		log.Warnf(context.Background(), "You are using version %s of VirtualBox. If you encounter issues, you might want to upgrade to version 5 at https://www.virtualbox.org", version)
	}
