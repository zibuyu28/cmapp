...
import (
	...
	"github.com/zibuyu28/cmapp/mrobot/drivers/virtualbox/version"
	"io"
	"io/ioutil"
	"net"
	"net/http"
...
func (b *b2dReleaseGetter) getReleaseURL(apiURL string) (string, error) {
	...
	log.Infof(context.Background(),"Latest release for %s/%s/%s is %s", host, org, repo, tag)
...
func (b *b2dISO) version() (string, error) {
	...
	log.Debugf(context.Background(),"local Boot2Docker ISO version: %s", vers)
	return vers, nil
}
...
func (b *B2dUtils) DownloadISO(dir, file, isoURL string) error {
	log.Infof(context.Background(),"Downloading %s from %s...", b.path(), isoURL)
...
func (b *B2dUtils) UpdateISOCache(isoURL string) error {
	...
	if _, err := os.Stat(b.imgCachePath); os.IsNotExist(err) {
		log.Infof(context.Background(),"Image cache directory does not exist, creating it at %s...", b.imgCachePath)
	...
	if isoURL != "" {
		if exists {
			...
			log.Warnf(context.Background(),"Boot2Docker URL was explicitly set to %q at create time, so Docker Machine cannot upgrade this machine to the latest version.", isoURL)
		}
	...
	if !exists {
		log.Info(context.Background(),"No default Boot2Docker ISO found locally, downloading the latest release...")
		return b.DownloadLatestBoot2Docker("")
	}

	latest := b.isLatest()
	if !latest {
		log.Info(context.Background(),"Default Boot2Docker ISO is out-of-date, downloading the latest release...")
		return b.DownloadLatestBoot2Docker("")
	}
...
func (b *B2dUtils) CopyIsoToMachineDir(isoURL, machineName string) error {
	...
	if isoURL == "" {
		log.Infof(context.Background(),"Copying %s to %s...", b.path(), machineIsoPath)
...
func (b *B2dUtils) isLatest() bool {
	...
	if err != nil {
		log.Warnf(context.Background(),"Unable to get the local Boot2Docker ISO version: %v", err)
		return false
	}

	latestVer, err := b.getReleaseTag("")
	if err != nil {
		log.Warnf(context.Background(),"Unable to get the latest Boot2Docker ISO release version: %v", err)
		return true
	}
...
func MakeDiskImage(publicSSHKeyPath string) (*bytes.Buffer, error) {
	...
	log.Debug(context.Background(),"Writing magic tar header")
	...
	log.Debug(context.Background(),"Writing SSH key tar header")
